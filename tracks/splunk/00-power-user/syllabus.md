...
Sections 1.0 and 2.0 ingested 2026-07-18: notes, cards, quiz, and a
hands-on lab exist (`notes/`, `cards/`, `quiz/1.0-2.0-quiz.yaml`,
`labs/1.0-2.0-lab.md`). Marked `[~]` (drafted, not yet drilled to mastery)
rather than `[x]` — that flips only once `/learn` sessions show consistent
recall.

## Next step

Run `/learn-ingest splunk 00-power-user` again to continue with section 3.0
(Correlating Events — transactions, the largest single section at 15%), then
proceed through 4.0–10.0 in blueprint order.
